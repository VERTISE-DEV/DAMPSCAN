...
export function partnerNamed(text) {
  const hay = ` ${String(text || '').toLowerCase()} `;
  const found = PEOPLE.filter((p) => PARTNER_NAMES[p].some((n) => new RegExp(`\\b${n}\\b`).test(hay)));
 ...
 * itself says whose money it is.
 */
export function autoTag(tx) {
  const type = String(tx.type || '').toUpperCase();
  const hay = haystack(tx);
  const out = tx.amountPence < 0;
  const partner = partnerNamed(hay);
