/**
 * Turning an uploaded statement into rows.
 *
 * Parse, drop what is already here, then for each new line: a learned rule if
 * one matches its description, otherwise a job if one clearly matches the
 * payment, otherwise the built-in guess. Everything that took a decision
 * records how it was decided, so a later rule can overwrite a guess and never
...
import { candidateJobs, syncJobPayments } from './ledger.js';
...
const COLUMNS = [
  ...
  'job_id', 'match_kind', 'rule_key'
];

/** Decide category, split and job for one new line. */
function decide(tx, rules, jobs) {
  ...
  const guess = autoTag(tx);
  return { key, category: guess.category, split: guess.split, jobId: null, matchKind: null };
}
 ...
 *          or {ok: false, error}
 */
export async function importStatement(text, filename) {
  ...
  const known = new Set((await query(
    'select fingerprint from bank_transactions where fingerprint = any($1::text[])',
    [unique.map((tx) => tx.fingerprint)]
  )).map((r) => r.fingerprint));
  ...
  const statement = await queryOne(
    `insert into bank_statements (filename, account, first_on, last_on, rows_seen, rows_added)
     values ($1, $2, $3, $4, $5, $6) returning id`,
    [filename || null, unique.find((tx) => tx.account)?.account || null,
      dates[0] || null, dates[dates.length - 1] || null, parsed.rows.length, fresh.length]
  );

  const rules = new Map((await query('select key, category, split from bank_rules'))
    .map((r) => [r.key, { category: r.category, split: r.split || [] }]));
  const jobs = fresh.some((tx) => tx.amountPence > 0) ? await candidateJobs(null) : [];

  const matchedJobs = [];
  let matched = 0;
  const values = fresh.map((tx) => {
    const d = decide(tx, rules, jobs);
    const share = shares(tx.amountPence, d.split);
    ...
    return [
      ...
      share.scott, share.tom, share.ben, share.tax,
      d.jobId, d.matchKind, d.key
    ];
  });

  for (let i = 0; i < values.length; i += BATCH) await insertBatch(values.slice(i, i + BATCH));
  await syncJobPayments(matchedJobs, 'forward');
