...
/**
 * A plain text body, for the one route that takes a file rather than JSON.
 * Vercel hands a text/* body over as a string and other types as a Buffer or
 * a stream, so all three are accepted. Over the cap returns null, decided
 * from the bytes as they arrive and not from a Content-Length a caller sets.
 */
export async function readText(req, maxBytes) {
  if (typeof req.body === 'string') return Buffer.byteLength(req.body) > maxBytes ? null : req.body;
  if (Buffer.isBuffer(req.body)) return req.body.length > maxBytes ? null : req.body.toString('utf8');
